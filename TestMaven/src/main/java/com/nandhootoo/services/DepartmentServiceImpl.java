...
public class DepartmentServiceImpl extends AbstractServiceImpl implements DepartmentService  {
	...
	public Department findByKey (Integer department) 
	...
	public void deleteItems (List<Integer> numbers, List<Department> departments) 
	{	EntityTransaction tx = null ;		
		int cntr = 0 ;			
		for (Integer anInt : numbers)	
		{	Department anDepartment = departments.get(anInt) ; 
			this.deleteKey(anDepartment.getId() ) ;
			cntr++ ; 
		}	
		System.out.println("" + cntr + " departments deleted" ); 	
	}	
	...
	{  
	 	try {			  
	 		String hqlDelete = "delete Department where id = :idString ";
